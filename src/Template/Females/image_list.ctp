    ...
    <table class="table table-striped">
        ...
        if(count($items)>0){
          ...
          <tr>
              ...
              <td class="actions">
				  <a href="<?php echo $this->Url->build(array('action'=>'viewImages',$types->id,$item->id)); ?>">Edit</a>
                  <?= $this->Form->postLink(__('Delete'), ['action' => 'delete',$item->icon,$types->id], ['confirm' => __('Are you sure you want to delete # {0}?', $item->id)]) ?>
              </td>
          </tr>
          <?php $i++; ?>
          <?php endforeach;
